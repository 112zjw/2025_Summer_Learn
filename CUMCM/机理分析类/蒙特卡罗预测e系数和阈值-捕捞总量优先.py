...
# ========== 单次模拟 ==========
def single_simulation(T, e1, e2, n_sim=30):
    catches = []
...
# ========== 主程序 ==========
def main():
    Ts  = np.linspace(0.4, 0.6, 11) * params.K   # 阈值
    e1s = np.linspace(0.1, 1.0, 10)              # e1
    e2s = np.linspace(0.1, 1.0, 10)              # e2（独立）
    ...
    df.to_csv('Te1e2_独立扫描.csv', index=False, encoding='utf-8-sig')

    best = df.loc[df['总捕捞量'].idxmax()]
    print("\n最优组合（总捕捞量最大）")
    print("="*50)
    print(f"阈值 T = {best['T']:.1f} ({best['T']/params.K:.2f}K)")
    ...
    sc = ax.scatter(T_all, e1_all, e2_all,
                    c=catch_all, cmap='plasma', alpha=0.7)
    ax.scatter(best['T'], best['e1'], best['e2'],
               color='red', s=120, edgecolor='k', label='最优点')
    ax.set_xlabel('阈值 T')
    ax.set_ylabel('e1')
    ax.set_zlabel('e2')
    ax.set_title('(T, e1, e2) 与总捕捞量（独立扫描）')
    fig.colorbar(sc, ax=ax, shrink=0.5, label='总捕捞量')
    ax.legend()
    plt.tight_layout()
    plt.savefig('3D_独立扫描.png', dpi=300)
    plt.show()
